...
def parse_args():
    ...
    # Algorithm
    parser.add_argument("--env-id", type=str, default="Gravitar-v5",
    ...
    parser.add_argument("--add_noise", type=lambda x: bool(strtobool(x)), default=True, nargs="?", const=True, help="if add noise")
    parser.add_argument("--bi_noise", type=lambda x: bool(strtobool(x)), default=False, nargs="?", const=True, help="if use bidirectional noise")
    parser.add_argument("--random-noise", type=lambda x: bool(strtobool(x)), default=False, nargs="?", const=False, help="if add noise")

    parser.add_argument("--rate", type=float, default=0.5, help="top-k")
    ...
    parser.add_argument("--noise_w", type=float, default=0.01, help="noise weight")
...
if __name__ == "__main__":
    ...
    if args.add_noise:
        ...
        if not args.bi_noise:
            ...
            run_name += f"-bi"
    run_name += f"_lr{args.learning_rate}__{args.seed}__{int(time.time())}"
    if args.track:
        import wandb
    ...
    # noise_dist = torch.distributions.normal.Normal(args.mean, args.std)
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            max_step_reward = max(rewards[step]).item()
            if max_step_reward > max_reward_visit[0]:
                if max_reward_visit[0] == -999:
                    max_reward_visit[0] = max_step_reward
                else:
                    max_reward_visit[1] = max_reward_visit[0]
                    max_reward_visit[0] = max_step_reward
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.add_noise:
                    ...
                    sorted_values, sorted_indices = torch.sort(torch.flatten(torch.abs(td)), descending=True)
                    num = int(len(mb_inds) * args.rate)

                    if args.random_noise:
                        top_k_indices = torch.randperm(len(mb_inds))[:num]
                    else:
                    ...
                    scale_noise = scale * noise
                    b_returns_select[top_k_indices] += scale_noise[top_k_indices]
