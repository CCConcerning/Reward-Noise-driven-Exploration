...
def parse_args():
    ...
    parser.add_argument("--save-rle", type=lambda x: bool(strtobool(x)), default=True, nargs="?", const=True,
...
class Agent(nn.Module):
    def __init__(self, envs, rle_net):
        ...
        self.critic_int = layer_init(nn.Linear(448, 1), std=0.01)  # rle_add
    ...
    def get_action_and_value(self, x, reward, goal, action=None, deterministic=False):
        ...
        goal_hidden = self.goal_encoder(goal)   # rle_add
        ...
        return (
            ...
            self.critic_int(features + hidden),   # rle_add
        )
